...
class TerminalPane extends BaseWidget {
  ...
  _ensurePty() {
    ...
    self._pty.onData(function (data) {
      ...
      // Flush any capability-query responses back to the PTY before we
      // render. fish's startup probe (DA1, XTVERSION, kitty keyboard
      // query, DCS XTGETTCAP) blocks the prompt for ~2s without these
      // -- and prints a "could not read response to Primary Device
      // Attribute query" warning -- so doing this synchronously after
      // each onData chunk cuts startup latency to near-zero.
      if (self._buf.responses) {
        try { self._pty.write(self._buf.responses); } catch (e) {}
        self._buf.responses = '';
      }
