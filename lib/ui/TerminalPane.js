...
class TerminalPane extends BaseWidget {
  ...
  runCommand(command) {
    ...
    return new Promise(function (resolve) {
      ...
      // Pipe chunks through ansi.chunkToTags so we (a) render SGR colors
      // and (b) escape blessed-meaningful braces -- subprocess output
      // commonly contains literal `{...}` text (e.g. `cat README.md` over
      // slap's own README) that would otherwise crash blessed's tag
      // parser inside scrollback.setContent. State carries across chunks
      // so a SGR reset that lands at a chunk boundary still applies.
      var stdoutState = ansi._initialState();
      var stderrState = ansi._initialState();
      function streamLines(state, buf) {
        var result = ansi.chunkToTags(buf.toString().replace(/\r\n?/g, '\n'), state);
        // Mutate the caller's state in place by copying fields.
        Object.assign(state, result[1]);
        var rendered = result[0].replace(/\n$/, '');
        if (!rendered) return;
        rendered.split('\n').forEach(function (line) { self.appendLine(line); });
      }

      child.stdout.on('data', function (buf) { streamLines(stdoutState, buf); });
      child.stderr.on('data', function (buf) { streamLines(stderrState, buf); });
