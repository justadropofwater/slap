...
class EditorPane extends Pane {
  constructor(opts) {
    super(_.merge({}, Slap.global.options.editorPane, opts));
    var self = this;
  ...
  setCurrent() {
    var self = this;
    super.setCurrent.apply(self, arguments);
    self.editor.focus();
    return self;
  }
  ...
  close() {
    ...
    return super.close.apply(self, arguments);
  }
  ...
  save(path) {
    ...
    return editor.save(path, slap.fileBrowser.cwd)
      .then(function (result) { header.message("saved to " + editor.textBuf.getPath(), 'success'); return result; })
      .catch(function (err) {
  ...
  _initHandlers() {
    ...
    self.on('element keypress', function (el, ch, key) {
      switch (self.resolveBinding(key)) {
        case 'save': if (!editor.readOnly()) editor.textBuf.getPath() ? self.save().catch(function(){}) : self.saveAsForm.show(); return false;
    ...
    var refreshGitDiff = _.debounce(function () { self._refreshGitDiff(); }, 250);
    ['onDidChange', 'onDidChangePath'].forEach(function (prop) {
      editor.textBuf[prop](function () {
        slap.header.render();
        if (slap.header.refreshGit) slap.header.refreshGit();
        refreshGitDiff();
        slap.paneList.update();
      });
    });
    editor.textBuf.onDidChangePath(function () { self._refreshGitDiff(); });
  ...
  _refreshGitDiff() {
    var self = this;
    var editor = self.editor;
    var filepath = editor.textBuf.getPath();
    if (!filepath) {
      if (editor.gitDiff) { editor.gitDiff = null; editor._updateContent(); }
      return;
    }
    var cwd = self.screen && self.screen.slap && self.screen.slap.fileBrowser
      ? self.screen.slap.fileBrowser.cwd : process.cwd();
    git.getLineDiff(cwd, filepath).then(function (diff) {
      editor.gitDiff = diff;
      editor._updateContent();
    }).catch(function () { /* not a repo or git failed; leave gitDiff alone */ });
  }
}
