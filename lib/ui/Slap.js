...
class Slap extends BaseWidget {
  ...
  _toggleTerminal() {
    ...
    if (self.terminal.visible) {
      ...
      self.terminal.show();
      // Pull keyboard focus into the terminal pane itself. The pane is
      // focusable (constructor sets focusable: true); BaseWidget's
      // element-keypress focus-traversal handler is intentionally not
      // wired up by TerminalPane._initHandlers, so Tab / Shift-Tab go
      // straight to the PTY for shell-side completion / history walks.
      self.terminal.focus();
    }
