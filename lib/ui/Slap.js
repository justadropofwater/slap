...
class Slap extends BaseWidget {
  constructor(opts) {
    ...
    self.terminal = new TerminalPane({parent: self});
  }
        ...
        .catch(function (err) {
          ...
          switch ((err.cause || err).code) {
            case 'EACCES':
              ...
              break;
            case 'EISDIR':
              self.fileBrowser.refresh(filePath, function(){});
              self.fileBrowser.focus();
              break;
            default: throw err;
          }
  ...
  quit() {
    ...
    self.panes.forEach(function(p) { p.detach(); });
  ...
  _toggleTerminal() {
    var self = this;
    var headerBottom = self.header.options.headerPosition === 'bottom' ? 1 : 0;
    if (self.terminal.visible) {
      self.terminal.hide();
      var current = self.getCurrentPane();
      if (current) current.focus();
      else if (self.fileBrowser.visible) self.fileBrowser.focus();
    } else {
      self.terminal.show();
      // Pull keyboard focus into the terminal pane's render surface.
      // We focus `view` rather than the pane itself because (a) blessed
      // focuses view on mouse click anyway, so keeping both paths
      // converging on view is consistent; and (b) the keypress handler
      // forwarding bytes to the PTY lives on view -- focusing the pane
      // would mean the first few keystrokes after F12 land on the wrong
      // emitter. terminal.hasFocus() still returns true with view focused
      // (descendant check), so slap's element-keypress gate continues to
      // suppress slap-level bindings.
      self.terminal.view.focus();
    }
    var paneBottom = self.terminal.visible ? self.terminal.height : headerBottom;
    self.panes.forEach(function (pane) {
      pane.bottom = paneBottom;
      if (pane.editor) pane.editor._updateContent();
    });
    self.fileBrowser.bottom = paneBottom;
    self.screen.render();
    return self;
  }
  ...
  _initHandlers() {
    ...
    self.on('element keypress', function (el, ch, key) {
      ...
      // While the terminal pane has focus, every key (except the toggle)
      // belongs to the PTY. The TerminalPane's own `keypress` handler has
      // already forwarded this key; here we just bail out so neither slap's
      // bindings nor BaseWidget's focus-traversal handler (registered later
      // in this method via super._initHandlers) fire. Returning false stops
      // further listeners on this 'element keypress' emitter.
      if (self.terminal && self.terminal.visible && self.terminal.hasFocus()) {
        if (binding === 'toggleTerminal') self._toggleTerminal();
        return false;
      }
    ...
    self.ready.then(function (slap) { return slap._initPlugins(); }).catch(function (err) {
...
var TerminalPane = require('./TerminalPane');
