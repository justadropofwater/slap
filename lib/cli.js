...
module.exports = function (options) {
  ...
  return Promise.all([
    Slap.getUserDir().catch(function () { return undefined; }),
    process.stdin.isTTY ? Promise.resolve() : readAsync(process.stdin)
  ]).then(function (results) {
    var userDir = results[0];
    var stdin = results[1];
    ...
    opts.screenOpts.input = ttys.stdin;
    ...
    Promise.all(opts._.map(function (path, i) {
      ...
      util.logger.error("error opening files: " + (err.stack || err));
    });
