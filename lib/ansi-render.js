  ...
  while (i < n) {
    ...
    if (ch === '\x1b') {
      ...
      if (next === '[') {
        ...
        // bracketed paste, terminfo init like CSI ?1034h, keypad
      ...
      if (next === ']' || next === 'P' || next === 'X' || next === '^' || next === '_') {
        ...
        // also emit DCS terminfo queries via XTGETTCAP. None of these are
        // useful to the renderer; dropping the entire envelope (header,
        // payload, terminator) keeps the visible output clean.
        var stringEnd = i + 2;
        while (stringEnd < n) {
          if (input[stringEnd] === '\x07') { stringEnd++; break; }
        ...
        if (stringEnd > n) break; // incomplete; pick up next chunk
        i = stringEnd;
        continue;
      }
