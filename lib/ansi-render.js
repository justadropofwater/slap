...
function createBuffer(opts) {
  ...
  return {
    ...
    cells: [],
    cursor: 0,
    // Saved cursor column for ESC 7 / ESC 8 (DECSC / DECRC) and
    // CSI s / CSI u (SCO save/restore). fish's autosuggestion uses these
    // heavily: save -> paint suggestion in dim -> restore so subsequent
    // keystrokes overwrite the suggestion. Without honoring the
    // save/restore pair we'd leave the cursor at the end of the painted
    // suggestion, and the user's next keystroke would land AFTER the
    // suggestion instead of overwriting it (visible bug: typing "ls"
    // produces "lsls" or "lslsls" because each keystroke appended).
    savedCursor: 0,
    sgr: initialSgr(),
  ...
  while (i < n) {
    ...
    if (ch === '\x1b') {
      ...
      if (next === '[') {
        ...
        } else if (finalCh === 's' && paramStr[0] !== '?') {
          // SCO save cursor (CSI s). The CSI ?...s variants are private
          // mode toggles (e.g. CSI ?1004s saves bracketed-paste state)
          // and we deliberately skip them here.
          buf.savedCursor = buf.cursor;
        } else if (finalCh === 'u') {
          // SCO restore cursor (CSI u). Pair with CSI s above.
          // (CSI ?u kitty-keyboard query is handled in its own branch.)
          buf.cursor = buf.savedCursor;
        } else if (finalCh === 'A' || finalCh === 'B') {
          // Cursor up (A) / down (B). The line-buffer model has no row
          // tracking, so multi-row moves are intentionally no-ops --
          // multi-line UI like vim/htop still won't render correctly.
        } else if (finalCh === 'C') {
          // Cursor forward n columns (default 1). Allow advancing past
          // cells.length; the gap renders as spaces via renderCells's
          // falsy-cell branch.
          buf.cursor += params[0] || 1;
        } else if (finalCh === 'D') {
          // Cursor backward n columns (default 1).
          buf.cursor = Math.max(0, buf.cursor - (params[0] || 1));
        } else if (finalCh === 'G') {
          // Cursor absolute column (1-indexed in the protocol).
          buf.cursor = Math.max(0, (params[0] || 1) - 1);
        } else if (finalCh === 'H' || finalCh === 'f') {
          // Cursor position row;col. Honor only when row is 1 or
          // unspecified -- applying a row > 1 in the line-buffer model
          // would corrupt prompt rendering.
          var row = params[0] || 1;
          var col = params[1] || 1;
          if (row === 1) buf.cursor = Math.max(0, col - 1);
        }
      ...
      if (next != null) {
        ...
        // (DECPNM normal keypad), ESC 7/8 (DECSC/DECRC save/restore
        // cursor), ESC c (RIS reset), ESC D/E/H/M (IND/NEL/HTS/RI),
        // ESC ( / ) / * / + 0 (character-set designation), and so on.
        //
        // ESC 7 / ESC 8 is the one pair we actually act on -- fish's
        // autosuggestion (and zsh's similar features) saves the cursor,
        // paints the suggestion in dim, then restores so the user's next
        // keystroke overwrites the suggestion. Without honoring the
        // pair, "ls" became "lsls" because each keystroke landed past
        // the painted suggestion instead of on top of it.
        if (next === '7') buf.savedCursor = buf.cursor;
        else if (next === '8') buf.cursor = buf.savedCursor;
        // Other single-byte ESC sequences are dropped (both bytes).
        i += 2;
        continue;
      }
