...
static void MakeCallbackInMainThread(uv_async_t* handle, int status) {
  ...
  if (!g_callback.IsEmpty()) {
    ...
    Nan::Call(Nan::New(g_callback), Nan::GetCurrentContext()->Global(), 4, argv);
  }
...
NAN_METHOD(Watch) {
  ...
  Local<String> path = Nan::To<String>(info[0]).ToLocalChecked();
  String::Utf8Value path_str(isolate, path);
  WatcherHandle handle = PlatformWatch(*path_str);
  if (!PlatformIsHandleValid(handle)) {
    ...
    if (error_number != 0) {
      err_obj->Set(context,
                   ...
                   Nan::New<v8::Integer>(error_number)).Check();
#if NODE_VERSION_AT_LEAST(0, 11, 5)
      err_obj->Set(context,
                   Nan::New<v8::String>("code").ToLocalChecked(),
                   Nan::New<v8::String>(uv_err_name(-error_number)).ToLocalChecked()).Check();
#endif
    }
